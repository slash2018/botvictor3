	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					...
					levelnol: '*error* 0 °-°',
					error: {
				stick: '*Falha, tente novamente ^_^*',
				Iv: '𝐝𝐞𝐬𝐜𝐮𝐥𝐩𝐞 𝐥𝐢𝐧𝐤 𝐢𝐧𝐯𝐚𝐥𝐢𝐝𝐨☹️'
				},
				only: {
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		setTimeout( () => {
	        ...
	        client.updatePresence(from, Presence.composing)
	        tujuh = fs.readFileSync('./assets/bott.mp3');
	        client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
    }
          ...
          case 'snack':
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'modonsfw':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('( ͡° ͜ʖ ͡°)')
					if ((args[0]) === 'on') {
						if (isNsfw) return reply('O modo nsfw já está ativo')
						nsfw.push(from)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
						reply(`\`\`\`✓Ativado com sucesso o modo nsfw no grupo ( ͡° ͜ʖ ͡°)\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
				...
				case 'loli2':
					...
					client.sendMessage(from, nyeee, image, { caption: 'Onii chan baka!!', quoted: mek })
					break
					case 'xd':				
		...
		case 'bpfont':
			...
			break 
				...
				uptime = process.uptime()
                    client.sendMessage(from, `Velocidade: *${latensi.toFixed(4)} _Segundos_*\nDispositivo: *𝐏𝐂 𝐆𝐀𝐌𝐄𝐑 𝐏𝐈𝐂𝐇𝐀𝐔*\nRAM/ARMAZENAMENTO: *𝟑𝟐/𝟓𝐓𝐁*\nOperadora: *𝐕𝐈𝐕𝐎 𝐅𝐈𝐁𝐑𝐀*\nRede: *𝐖𝐋𝐀𝐍 𝟐𝟎𝟎𝐌𝐁𝐬*\nStatus: *𝐀𝐓𝐈𝐕𝐎*\nTipo do bot: *𝐂𝐎𝐌𝐄𝐃𝐎𝐑 𝐃𝐄 𝐂𝐀𝐒𝐀𝐃𝐀𝐒*\n\n*O bot esteve ativo por*\n*${kyun(uptime)}*`, text, { quoted: mek})
                    break
				case 'delete':
				case 'del':
				case 'd':  
...
break
case 'pqp':
tujuh = fs.readFileSync('./assets/auo.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'bv':
tujuh = fs.readFileSync('./assets/bv.mp3');
...
break
           ...
           case 'hobby':
					...
					hobby = body.slice(1)
					const hob =['Suspirando no jogo', 'Ngocokin Doi', 'Perseguindo sua antiga mídia social', 'Você não tem um hobby de awokawok', 'Cozinhar', 'Ajudar Atok', 'Mabar', 'Nobar', 'Sosmedtan','Ajudar outras pessoas','Assistir Anime','Assistir Drakor','Andar de motocicleta','Cantando','Dança','Batendo','Desenhar','Fotografias pouco claras','Jogo de Maen' , 'Falando sozinho']
...
// recursos anime, random @ADM VICTOR domina irmão
          ...
          reply('Espere..')
				...
				if (!isOwner) return reply(mess.only.ownerB)
					...
					client.sendMessage(from, ssha, image, {caption: 'kkkkkk', quoted: mek})
					break
				case 'minion':
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
					...
					client.sendMessage(from, minn, image, {caption: 'kkkkkk', quoted: mek})
					break
				case 'neon':
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
					...
					client.sendMessage(from, nooe, image, {caption: 'kkkkkk', quoted: mek})
					break
				case 'neongreen': 
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
					...
					client.sendMessage(from, gree, image, {caption: 'kkkkkk', quoted: mek})
					break
				case 'neon2':
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
				...
				if (!isOwner) return reply(mess.only.ownerB)
					...
					client.sendMessage(from, tigaa, image, {caption: 'kkkkkk', quoted: mek})
					break
				case 'walpaperanime':
				    try {
			    	if (!isOwner) return reply(mess.only.ownerB)
			...
			reply(mess.wait)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ 𝐓𝐑𝐀𝐍𝐒𝐌𝐈𝐒𝐒𝐀𝐎 𝐃𝐄 𝐀𝐕𝐈𝐒𝐎 ]\n\n${body.slice(4)}`)
						}
						reply('Transmissão enviada com sucesso')
					}
					break
